...
class VulkanApplication
{
    ...
    this(string appName, HINSTANCE hInstance)
    {
        name = appName;
    ...
    private void initVulkan()
    {
        ...
        {
            res = vkAllocateCommandBuffers(device, &cmd, &buffers[i].cmd);
            assert(res == VkResult.VK_SUCCESS);
        }
    ...
    void enumeratePhysicalDevices()
    {
        ...
        pfnGetPhysicalDeviceSurfaceSupportKHR = 
            cast(PFN_vkGetPhysicalDeviceSurfaceSupportKHR)vkGetInstanceProcAddr(
                inst, toStringz("vkGetPhysicalDeviceSurfaceSupportKHR"));
        assert(pfnGetPhysicalDeviceSurfaceSupportKHR !is null);
        
        pfnGetPhysicalDeviceSurfaceFormatsKHR = 
            cast(PFN_vkGetPhysicalDeviceSurfaceFormatsKHR)vkGetInstanceProcAddr(
                inst, toStringz("vkGetPhysicalDeviceSurfaceFormatsKHR"));
        //assert(pfnGetPhysicalDeviceSurfaceFormatsKHR !is null);
    }
    ...
    void createWin32Window()
    {
        ...
        win_class.cbSize = WNDCLASSEX.sizeof;
        win_class.style = CS_HREDRAW | CS_VREDRAW;
        win_class.lpfnWndProc = cast(typeof(WNDCLASSEX.lpfnWndProc))&WndProc;
        win_class.cbClsExtra = 0;
        win_class.cbWndExtra = 0;
        win_class.hInstance = connection;
        win_class.hIcon = LoadIcon(null, IDI_APPLICATION);
        win_class.hCursor = LoadCursor(null, IDC_ARROW);
        win_class.hbrBackground = cast(void*)(COLOR_WINDOW+1); 
        win_class.lpszMenuName = null;
        win_class.lpszClassName = cast(wchar*)"MyWindowClass"w;
        win_class.hIconSm = LoadIcon(null, IDI_WINLOGO);
        
        // Register window class:
        if (!RegisterClassEx(&win_class))
        {
            throw new Exception("Failed to register win_class");
        }
    ...
    void createWin32Surface()
    {
        ...
        {
            //supportsPresent[0] = vkGetPhysicalDeviceWin32PresentationSupportKHR(gpu, i);
    ...
    void initSwapchain()
    {
        pfnCreateSwapchainKHR = 
            cast(PFN_vkCreateSwapchainKHR)vkGetInstanceProcAddr(
                inst, "vkCreateSwapchainKHR");
        assert(pfnCreateSwapchainKHR !is null);
        
        pfnDestroySwapchainKHR = 
            cast(PFN_vkDestroySwapchainKHR)vkGetInstanceProcAddr(
                inst, "vkDestroySwapchainKHR");
        assert(pfnDestroySwapchainKHR !is null);
        
        pfnGetSwapchainImagesKHR = 
            cast(PFN_vkGetSwapchainImagesKHR)vkGetInstanceProcAddr(
                inst, "vkGetSwapchainImagesKHR");
        assert(pfnGetSwapchainImagesKHR !is null);
        
        pfnAcquireNextImageKHR = 
            cast(PFN_vkAcquireNextImageKHR)vkGetInstanceProcAddr(
                inst, "vkAcquireNextImageKHR");
        assert(pfnAcquireNextImageKHR !is null);
        
        pfnQueuePresentKHR = 
            cast(PFN_vkQueuePresentKHR)vkGetInstanceProcAddr(
                inst, "vkQueuePresentKHR");
        assert(pfnQueuePresentKHR !is null);
        
        pfnDestroySemaphore = 
            cast(PFN_vkDestroySemaphore)vkGetInstanceProcAddr(
                inst, "vkDestroySemaphore");
        assert(pfnDestroySemaphore !is null);
    ...
    */
    ...
    static void draw()
    {
        ...
        auto res = vkCreateSemaphore(app.device, 
            &presentCompleteSemaphoreCreateInfo, null, &presentCompleteSemaphore);
        assert(res == VkResult.VK_SUCCESS); 
        
        res = vkAcquireNextImageKHR(app.device, app.swapchain, ulong.max,
        ...
        app.setImageLayout(app.buffers[app.current_buffer].image,
            VkImageAspectFlagBits.VK_IMAGE_ASPECT_COLOR_BIT,
            VkImageLayout.VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
        ...
        VkSubmitInfo submit_info =
        {
            ...
            commandBufferCount: 1,
            pCommandBuffers: &app.buffers[app.current_buffer].cmd,
            signalSemaphoreCount: 0,
            pSignalSemaphores: null
        };
        
        res = vkQueueSubmit(app.queue, 1, &submit_info, nullFence);
        assert(res == VkResult.VK_SUCCESS); 
        ...
        VkPresentInfoKHR present =
        {
            ...
            pImageIndices: &app.current_buffer,
        };
        
        res = vkQueuePresentKHR(app.queue, &present);
        if (res == VkResult.VK_ERROR_OUT_OF_DATE_KHR)
        {
            // demo->swapchain is out of date (e.g. the window was resized) and
            // must be recreated:
        ...
        res = vkQueueWaitIdle(app.queue);
        assert(res == VkResult.VK_SUCCESS); 
        
        vkDestroySemaphore(app.device, presentCompleteSemaphore, null);
    }
    ...
    static extern(Windows) LRESULT WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        switch (uMsg)
        {
            case WM_CLOSE:
